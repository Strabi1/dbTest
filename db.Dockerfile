FROM ubuntu:20.04

ENV LANG=C.UTF-8 \
    LC_ALL=C.UTF-8 \
    DEBIAN_FRONTEND=noninteractive

RUN apt update && \
    apt install -y --no-install-recommends \
        ca-certificates unzip sudo htop \
        build-essential g++ gcc make \
        git \
        python3 python3-pip \
        mariadb-server \
        libmariadb-dev \
        mariadb-client \
        # libboost-all-dev \
        libcpprest-dev \
        #libmysqlclient-dev \
        libssl-dev
RUN apt clean && rm -rf /var/lib/apt/lists/*

RUN git config --global --add safe.directory /media

# MariaDB indítása és inicializálása
# RUN service mysql start && \
#     mysql -e "CREATE DATABASE food_db;" && \
#     mysql -e "CREATE USER 'user'@'%' IDENTIFIED BY 'password';" && \
#     mysql -e "GRANT ALL PRIVILEGES ON food_db.* TO 'user'@'%';" && \
#     mysql -e "FLUSH PRIVILEGES;"

# RUN mysqld --skip-networking & \
#     sleep 10 && \
#     mysql -u root -e "CREATE DATABASE food_db;" && \
#     mysql -u root -e "CREATE USER 'user'@'%' IDENTIFIED BY 'password';" && \
#     mysql -u root -e "GRANT ALL PRIVILEGES ON food_db.* TO 'user'@'%';" && \
#     mysql -u root -e "FLUSH PRIVILEGES;"

RUN mkdir -p /media/database
COPY ./init.sql /media/database

# RUN mysql food_db < /media/database/init.sql

WORKDIR /media/database
